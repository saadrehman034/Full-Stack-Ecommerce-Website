  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from("products").insert({
        ...
        images: filteredImages.length ? filteredImages : [],
  ...
  return (
    <div className="mx-auto max-w-3xl p-8">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-syne text-2xl font-bold">New Product</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <fieldset className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-border/30 dark:bg-[#111] space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input required value={form.name} onChange={e => set("name", e.target.value)}
                placeholder="e.g. Himalayan Pink Salt 500g"
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => set("description", e.target.value)}
              ...
              className="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring resize-none" />
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input type="number" step="0.01" min="0" value={form.compare_price} onChange={e => set("compare_price", e.target.value)}
                placeholder="0.00 (original price for sale badge)"
                className="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring" />
            </div>
          </div>
        </fieldset>
        ...
        <fieldset className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-border/30 dark:bg-[#111] space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium">Low Stock Alert</label>
            ...
            <div key={i} className="space-y-2">
              <div className="flex gap-2">
                ...
                <input type="file" accept="image/*"
                  ...
                  className="hidden"
                  onChange={e => { const file = e.target.files?.[0]; if (file) uploadImage(file, i); e.target.value = ""; }} />
                {images.length > 1 && (
